...
print "<tr>
	<td><label style=\"width: 100%;\"><input style=\"float: left;\" type=\"radio\" name=\"plan\" id=\"plan\" value=\"".$row['id']."\" checked /> <div style=\"padding: 4px 0 4px 20px; background-color: #eeeeee;\"><b style=\"float: center;\">".$row['name']."</b></div>";
		print "<div style=\"padding-left: 22px;\">Deposit amount is: $".$row['minsum']." - $".$row['maxsum']." at ".$row['percent']."% in ";
		if($row['period'] == 1) { print "day"; } elseif($row['period'] == 2) { print "неделю"; } elseif($row['period'] == 4) { print "час"; } else { print "месяц"; }
		print ", for a period  ".$row['days'];
		if($row['period'] == 4) { print " часов"; } elseif($row['period'] == 1) { print " days"; } elseif($row['period'] == 2) { print " недель"; } elseif($row['period'] == 3) { print " месяцев"; }
		print "</div></label></td>
</tr>
<tr>
